...
knn <- function (x, z, zout, k = 0, pw, reg = TRUE, nensemble = 100,
          tailcorrection = TRUE, tailprob = 0.25, tailfac = 0.2, extrap = TRUE)
{
  x <- as.matrix(x)
  n <- nrow(x)
  if (k == 0) {
  ...
  for (i in 1:nzout) {
    z_out <- matrix(rep(zout[i, ], nrow(z)), nrow = nrow(z),
                    byrow = "T")
    dtmp <- z_out - z
    ...
    if (reg) {
      if (!extrap) {
        ...
        xhat[i] <- sum((x[ord] + dtmp[ord, ] %*% t(Scond)) *
                         kern)
      }
    }
    if (!reg) {
