...
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(dirname(inputFile), paste0('NPRED_r100_Pseudo_v3'))) }) #,format(Sys.time(),"%Y-%m-%d %H %M")))) })
editor_options: 
...
path.out <- paste0(station_id,"/cli_mod_pseudo/")
...
Qsim_mon <- Qmon_sim %>% gather(group,Qm,3:(100+2)) %>% group_split(group, keep = F)  
Qsim_day <- Qday_sim %>% gather(group,Qd,4:(100+3)) %>% group_split(group, keep = F)  
summary(Qsim_mon[[1]])
...
val.st <- date_year[1]; val.end <- date_year[length(date_year)]
...
Qday_obs <- Qday_obs[!(Qday_obs$day == 29 & Qday_obs$month == 2), ]    
...
# Plot 1 realization of SIM annual TS
