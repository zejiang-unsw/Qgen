...
flag.ver <- switch(4, "","_v1","_v2","_v3")
flag.sel <- switch(2, "ALL","NPRED","WASP") # predictor selection method

flag.gcm <- switch (4,"_hist","_rcp85_near", "_rcp85_far", "_pseudo")
if(TRUE){
  ...
  date_cli1 <- date_cli[date_cli<as.Date(date_val[1])]; tail(date_cli1)
  ind_Qobs1 <- which(date_Qobs %in% date_cli1); range(date_Qobs[ind_Qobs1])
  ind_cli1 <- which(date_cli<as.Date(date_val[1])); range(date_cli[ind_cli1])
