...
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(dirname(inputFile), paste0('NPRED_GCM_r100_comp'))) }) #,format(Sys.time(),"%Y-%m-%d %H %M")))) })
editor_options: 
...
path.out <- paste0(station_id,"/cli_mod_hist_A/")#mod_",mod_id,"/") 
...
```{r seasonality}
out.Qmon <- list()
gcms <- c("hist_A","rcp85_near", "rcp85_far")
for(i in 1:length(gcms)) {
  flag.gcm <- gcms[i]
  print(flag.gcm)
  path.out <- paste0(station_id,"/cli_mod_",flag.gcm,"/")#mod_",mod_id,"/")
  Qsim_mon <- get(load(paste0(path.out, "Harz_Qmon_",flag.sel,"_r",nensemble,flag.ver,".Rdat"))) 
...
p.sc <- ggplot(subset(sc.plot1,gcm!="hist"), aes(x=group, y=sc, color=gcm)) +
  ...
  scale_y_continuous(limits=c(0.8,1.2), breaks = pretty_breaks()) +
