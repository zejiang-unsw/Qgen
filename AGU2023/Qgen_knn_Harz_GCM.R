...
gcms <- c("_hist","_rcp85_near", "_rcp85_far")[2:3]
for(flag.gcm in gcms){

  # output----
  path.out <- paste0(station_id,"/cli_mod",flag.gcm,"/")#"/mod_",mod_id,"/")
  ...
  k <- floor(0.5 + 3 * sqrt(length(unique(Qmon$year))));k
  ...
  k <- floor(0.5 + 3 * sqrt(length(unique(Qday$year))));k
