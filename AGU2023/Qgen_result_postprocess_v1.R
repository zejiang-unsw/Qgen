...
for(j in 1:length(vals)){
  flag.val <- vals[j]
  path.out <- paste0(station_id,"/cli_val_",flag.val,"/")
  print("validation")
  ...
  if(flag.save){
    ...
    path.out <- paste0(station_id,"_v1/cli_val_",flag.val,"/")
...
## future simulation----
out.Qmon <- list()
gcms <- c("hist_A","hist_A2","rcp85_near", "rcp85_far")
for(i in 1:length(gcms)) {
  # i <- 1
  flag.gcm <- gcms[i]
  print(flag.gcm)
  path.out <- paste0(station_id,"/cli_mod_",flag.gcm,"/")
  ...
  if(flag.save){
    path.out <- paste0(station_id,"_v1/cli_mod_",flag.gcm,"/")
    if (!dir.exists(path.out)){
      dir.create(path.out, recursive = T)
    } else {
      print("Dir already exists!")
